...
import com.baixc.ucenter.model.user.City;
import com.baixc.ucenter.service.user.CityService;
...
public class CityServiceImpl extends BaseService implements CityService {
	...
	public List<City> getCityList(Map<String,Object> params) {
		try{
			...
			List<City> list = this.sqlMapClientCommons.queryForList("bai_city.getCity", params);
			return list;
		}catch(Exception e){
	...
	public void delCity(int[] ids) {
		try{
			this.sqlMapClientCommons.delete("bai_city.delCity", ids);
		}catch(Exception e){
	...
	public void addCity(City city) {
		try{
			this.sqlMapClientCommons.insert("bai_city.addCity", city);			
	...
	public void editCity(City city) {
		try{
			this.sqlMapClientCommons.update("bai_city.editCity", city);			
	    }catch(DuplicateKeyException e){
			throw new RpcException("城市ID或编码重复", e);	
	    } catch (Exception e) {
